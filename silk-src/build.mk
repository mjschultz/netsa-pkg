# RCSIDENT("$SiLK: build.mk ccb5752c6abb 2016-12-02 16:50:22Z mthomas $")
#
...
########  LUA SUPPORT
#
#  Rules to compile *.lua files---with the Lua compiler $(LUAC)---and
#  to create C arrays from them via a "hexdump" script $(LUA2HEX)
#
LUAC = $(top_builddir)/lua/src/luac
LUA2HEX = $(top_srcdir)/src/libsilk/lua2hex.pl
#LUA2HEX_ARGS = --strip

#  When set, the $(sk__skip_luac) variable prevents rebuilding of
#  "foo.i" even if it is older than the corresponding "foo.lua" file.
#  That is, it makes those files behave like the .c files generated by
#  lex and yacc.  The variable is set when the following line is
#  uncommented and the user does NOT specify --enable-maintainer-mode.

#@MAINTAINER_MODE_FALSE@sk__skip_luac = test -f $@ ||

.lua.i:
	@if test -x $(LUAC) ; then : ; else cd $(top_builddir)/lua/src && $(MAKE) luac ; fi
	-@$(sk__skip_luac)rm -f $@
	$(AM_V_GEN)$(sk__skip_luac)$(PERL) $(LUA2HEX) --luac=$(LUAC) $(LUA2HEX_ARGS) $< $@
...
SILK_TESTDATA_IPFIX = $(top_builddir)/tests/made-ipfix-files
$(SILK_TESTDATA_IPFIX):
	cd $(top_builddir)/tests && $(MAKE) made-ipfix-files
