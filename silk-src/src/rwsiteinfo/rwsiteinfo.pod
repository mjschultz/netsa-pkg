...
examples is based on the sample F<silk.conf>.
...
default type for each class, but the default site has a single class.)
...
B<silk.conf(5)>, B<rwfilter(1)>, B<rwfglob(1)>,
B<silk(7)>, B<tzset(3)>, B<environ(7)>

=cut

$SiLK: rwsiteinfo.pod fc8761f1d6b8 2017-06-23 16:42:19Z mthomas $
