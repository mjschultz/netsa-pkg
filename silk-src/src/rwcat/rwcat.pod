...
will write the records to the standard output when stdout is not the
...
When the B<--xargs> switch is provided, B<rwcat> will read the names
...
=item B<--output-path>=I<FILE>

Write the SiLK Flow records to I<FILE>, which must not exist.  If the
switch is not provided or if I<FILE> is C<stdout>, flows are written
to the standard output.  If the name ends in C<.gz>, the output will
be compressed using B<gzip(1)>.
...
Causes B<rwcat> to read file names from I<FILENAME> or from the
standard input if I<FILENAME> is not provided.  The input should have
one file name per line.  B<rwcat> will open each file in turn and read
...
$SiLK: rwcat.pod cd09f0a5fcc6 2014-06-14 15:43:33Z mthomas $
