...
#
#
# RCSIDENT("$SiLK: rwflowpack-pack-silk-discard-when.pl 23e62811e29c 2016-11-16 15:30:29Z mthomas $")
...
local function pack_function (probe, rec)
  ...
  -- Discard (ignore) record when its source IP or destination IP
  -- matches probe.null
  if probe.null[saddr] or probe.null[daddr] then
    return
  end
