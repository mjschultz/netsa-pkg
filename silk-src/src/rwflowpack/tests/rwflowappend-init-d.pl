...
#
#
# RCSIDENT("$SiLK: rwflowappend-init-d.pl fc5a7e50648b 2015-09-23 19:30:01Z mthomas $")
...
while (<SRC>) {
    ...
    if (/^(ARCHIVE_DIR=).*/) {
        $daemon_conf_text .= $1 . '${statedirectory}/archive' . "\n";
        next;
    }
    ...
    if (/^(REJECT_HOURS_(PAST|FUTURE)=).*/) {
        $daemon_conf_text .= $1 . "5\n";
        next;
    }
    if (/^(APPEND_THREADS=).*/) {
        $daemon_conf_text .= $1 . "4\n";
        next;
    }
