...
#
#
# RCSIDENT("$SiLK: rwflowpack-pack-silk-ipv6.pl 23e62811e29c 2016-11-16 15:30:29Z mthomas $")
...
sub check_file
{
    ...
    # do the MD5 sums match?  Use sort since input has two files,
    # and we don't know which was processed first
    my $check_cmd
...
local ipblocks_internal = silk.ipset({ "192.168.x.x", "2001:db8:c0:a8::x:x" })
local ipblocks_external = silk.ipset({ "10.0.0.0/8", "2001:db8:a:x::x:x" })
local ipblocks_null = silk.ipset({ "172.16.0.0/13", "2001:db8:ac:10-17::x:x" })
