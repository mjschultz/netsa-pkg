...
#
#
# RCSIDENT("$SiLK: rwflowpack-pack-silk-discard-unless.pl 23e62811e29c 2016-11-16 15:30:29Z mthomas $")
...
-- Another set of IP addresses to keep
local ipblocks_other    = silk.ipwildcard("172.24.0.0/13")

-- create ipblocks_keep as the union of ipblocks_{internal,external,other}
local ipblocks_keep = silk.ipset({ipblocks_internal, ipblocks_external, ipblocks_other})
...
local function pack_function (probe, rec)
  ...
  -- Discard (ignore) record unless its source IP and destination IP
  -- match ipblocks_keep
  if not (ipblocks_keep[saddr] and ipblocks_keep[daddr]) then
    return
  end
