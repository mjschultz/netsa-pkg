...
#
#
# RCSIDENT("$SiLK: rwflowpack-pack-ipfix-ipv6.pl 23e62811e29c 2016-11-16 15:30:29Z mthomas $")
...
my $cmd = join " ", ("$SiLKTests::PYTHON $srcdir/tests/rwflowpack-daemon.py",
                     "--basedir=$tmpdir",
                     "--copy $file{v6data_ipfix}:incoming",
                     "--limit=501876",
                     "--",
                     $config_lua,
    );
...
verify_archived_files("$tmpdir/archive", $file{v6data_ipfix});
...
input = {
    ...
    probes = {
        P0 = {
            ...
            vars = {
                internal = silk.ipwildcard("2001:db8:c0:a8::x:x"),
                external = silk.ipwildcard("2001:db8:a:x::x:x"),
                null = silk.ipwildcard("2001:db8:ac:10-17::x:x"),
