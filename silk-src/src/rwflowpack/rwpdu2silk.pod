...
ends in C<.gz>, the file is uncompressed as it is read.  When the
B<--xargs> switch is provided, B<rwpdu2silk> reads the names of the
...
B<rwpdu2silk> exits with an error unless the SILK_CLOBBER environment
...
B<rwpdu2silk> to exit with an error.
...
Enable the default set of log-flags used by F<sensor.conf>: B<bad>,
...
to have one filename per line.  B<rwpdu2silk> opens each named file in
...
B<rwfileinfo(1)>, B<rwflowpack(8)>, B<sensor.conf(5)>, B<silk(7)>,
B<zlib(3)>
...
$SiLK: rwpdu2silk.pod d1637517606d 2017-06-23 16:51:31Z mthomas $
