...
When multiple probes use the same B<listen-on-port>, B<protocol>, and
B<listen-as-host> values, the B<accept-from-host> switch must be used
so that B<rwflowpack> may assign incoming records to a specified
probe.  When listening for NetFlow, this attribute would be the IP
address of the router as seen from the machine running B<rwflowpack>
or B<flowcap>.  (Prior to SiLK 3.10.1, the B<accept-from-host> command
...
=item B<quirks> { B<none> | { B<firewall-event> | B<zero-packets> | B<missing-ips> | B<nf9-sysuptime-seconds> ... } }
...
Work around an issue with NetFlow v9 records from a SonicWall
appliance where the sysUpTime field in the packet header is reported
in seconds instead of in milliseconds.  The incorrect units cause the
time stamps on flow records to future dated.  In addition, adjust the
time fields on single packet flow records.  I<Since SiLK 3.14.0.>
...
$SiLK: sensor.conf.pod a2684b2e4c23 2017-12-01 22:50:53Z mthomas $
