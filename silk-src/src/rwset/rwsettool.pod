...
provided, B<rwsettool> attempts to write the IPset to the standard
output when the standard output is not connected to a terminal.
...
Create a file that may hold only IPv4 adresses and is readable by all
versions of SiLK.

=item S< 3 >

Create a file that may hold IPv4 or IPv6 adresses and is readable by
...
Create a file that may hold only IPv6 adresses and is readable by
...
When using the B<--size> switch, the probably of selecting an
...
B<rwset(1)>, B<rwsetbuild(1)>, B<rwsetcat(1)>, B<rwbagbuild(1)>,
B<rwbagtool(1)>, B<rwfileinfo(1)>, B<silk(7)>, B<zlib(3)>
...
$SiLK: rwsettool.pod 88cee5c95d36 2016-10-31 16:57:21Z mthomas $
