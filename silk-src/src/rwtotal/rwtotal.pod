...
When the B<--xargs> switch is provided, B<rwtotal> will read the names
...
B<--output-path> switch has been used to redirect B<rwtotal>'s ASCII
output.

=item B<--output-path>=I<PATH>

Determine where the output of B<rwtotal> (ASCII text) is written.  If
...
Causes B<rwtotal> to read file names from I<FILENAME> or from the
standard input if I<FILENAME> is not provided.  The input should have
one file name per line.  B<rwtotal> will open each file in turn and
...
$SiLK: rwtotal.pod cd09f0a5fcc6 2014-06-14 15:43:33Z mthomas $
