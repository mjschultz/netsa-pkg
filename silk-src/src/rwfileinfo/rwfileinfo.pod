...
SiLK 4.0.0 introduced sidecar fields as a way to augment a SiLK Flow
record with additional data.  This field displays a description of the
sidecar fields that may be present on the (some of the) records in the
file.  For each file, its name and type are shown.  The IPFIX element
ID and enterprise ID for that field are printed when they are
...
B<rwfileinfo> understands IPFIX files.  When an IPFIX file is
specified on the command line, B<rwfileinfo> prints its format as
C<IPFIX>, the size of the file (C<file-size>), the number of records
in the file (C<count-records>), and the following fields:

=over 4

=item B<export-time>

Each IPFIX message has a timestamp, and this field shows the timestamp
of the first message in the IPFIX file.

=item B<schemas>

Each IPFIX template (which SiLK calls a schema) in the file is
printed.  The output for each schema consists of the template ID, the
number of fields in the schema, the length of a record uses the
schema, the number of (top-level) records in the file that use that
schema, and, for each information element in the schema, its name, its
enterprise ID if non-standard, its element id, and its length.
...
For an IPFIX file, the export time of the first message in the file.

=item schemas,21

For an IPFIX file, the schemas (templates) in the file and the number
of records that use each schema.
...
$SiLK: rwfileinfo.pod ff3b862e55a9 2017-06-23 18:46:19Z mthomas $
