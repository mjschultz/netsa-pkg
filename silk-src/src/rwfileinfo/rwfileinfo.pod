...
B<rwfileinfo> requires one or more file arguments to be given on the
command line.  To have B<rwfileinfo> read a file's contents from the
standard input, use C<-> or C<stdin> as a file argument.
...
(B<rwfileinfo> prints a field in order in which support for that field
was added to SiLK.  The field descriptions are presented here in a
more logical order.)
...
SiLK tools may use the zlib library (L<http://zlib.net/>) or the LZO
library (L<http://www.oberhumer.com/opensource/lzo/>) to compress the
...
$SiLK: rwfileinfo.pod caef286c367c 2016-06-03 19:23:54Z mthomas $
