...
it is read.  When the B<--xargs> switch is provided, B<rwcombine> will
...
Causes B<rwcombine> to read file names from I<FILENAME> or from the
standard input if I<FILENAME> is not provided.  The input should have
one file name per line.  B<rwcombine> will open each file in turn and
...
The output from B<rwcut(1)> shows the flow exporter split this
long-lived B<ssh> session into multiple flow records:
...
$SiLK: rwcombine.pod d85feea768f3 2014-09-24 22:02:33Z mthomas $
