...
When the B<--xargs> switch is provided, B<rwsort> reads the names of
...
Prior to SiLK 4.0, B<rwsort> supported an B<--input-pipe> switch whose
argument was either the path to a named pipe from which B<rwsort> read
records or the strings C<stdin> or C<-> to tell B<rwsort> to read from
...
C<stdin> or C<-> as a command-line argument.  When no input arguments
are specified, B<rwsort> automatically attempts to read data from its
standard input.
...
$SiLK: rwsort.pod 6ced2b9cf93b 2017-06-26 21:15:05Z mthomas $
