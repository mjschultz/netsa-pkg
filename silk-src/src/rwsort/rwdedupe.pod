...
is read.  When the B<--xargs> switch is provided, B<rwdedupe> reads
...
B<rwdedupe> exits with an error unless the SILK_CLOBBER environment
...
Attempting to write the binary output to a terminal causes B<rwdedupe>
...
to have one filename per line.  B<rwdedupe> opens each named file in
...
B<rwfilter(1)>, B<rwfileinfo(1)>, B<silk(7)>,
B<yaf(1)>, B<zlib(3)>

=cut

$SiLK: rwdedupe.pod d1637517606d 2017-06-23 16:51:31Z mthomas $
