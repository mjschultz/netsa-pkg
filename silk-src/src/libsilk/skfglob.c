...
RCSIDENT("$SiLK: skfglob.c efd886457770 2017-06-21 18:43:23Z mthomas $");
...
static int fglobAdjustCountersFlowtype(sk_fglob_t *fglob);
static int fglobParseClassAndType(sk_fglob_t *fglob);

static int fglobInit(sk_fglob_t *fglob);
static int fglobLoadSiteConfig(sk_fglob_t *fglob);
static int fglobInitTimes(sk_fglob_t *fglob);
static int fglobHandler(clientData cData, int opt_index, char *opt_arg);
static void fglobEnableAllSensors(sk_fglob_t *fglob);
static int fglobParseSensors(sk_fglob_t *fglob, sk_bitmap_t **sensor_bits_ptr);
    ...
    sk_fglob_t        **fglob_parm)
{
    ...
    *fglob_parm = NULL;
    ...
    /* create the structure and the array to hold the option strings */
    fglob = sk_alloc(sk_fglob_t);
    fglob->fg_option = sk_alloc_array(char *, num_opts);

    /* register the options */
    if (skOptionsRegister(fglobOptions, &fglobHandler, (clientData)fglob)
        || sksiteOptionsRegister(SK_SITE_FLAG_CONFIG_FILE))
    {
        skAppPrintErr("Unable to register options");
        free(fglob->fg_option);
        free(fglob);
        return -1;
    }
...
/*
 *    Print usage for the --data-rootdir switch and print the current
 *    value.
 */
static void
fglobUsageDataRootdir(
    FILE               *fh)
{
    ...
    sk_fglob_t        **fglob_parm)
{
    ...
    if (NULL == fglob_parm || NULL == *fglob_parm) {
        return;
    }
    fglob = *fglob_parm;
    *fglob_parm = NULL;
    ...
    free(fglob->fg_flowtype_list);
    if (fglob->fg_sensor_list) {
        for (i = sksiteFlowtypeGetMaxID(); i >= 0; --i) {
            free(fglob->fg_sensor_list[i]);
        }
        free(fglob->fg_sensor_list);
    }
    free(fglob->fg_sensor_count);
    ...
    size_t              bufsize)
{
    ...
    while (fglobAdjustCountersFlowtype(fglob)) {
         ...
         * for the file. */
        repo_key.flowtype_id = fglob->fg_flowtype_list[fglob->fg_flowtype_idx];
        repo_key.timestamp = fglob->fg_time_idx;
        repo_key.sensor_id =
            fglob->fg_sensor_list[fglob->fg_flowtype_idx][fglob->fg_sensor_idx];

        if (!sksiteGeneratePathname(buf, bufsize, &repo_key, ".gz", NULL,NULL))
        {
            continue;
        }
    ...
    sk_fglob_t         *fglob)
{
    ...
    fglob->fg_initialized = 1;

    /* load the site config */
    if (fglobLoadSiteConfig(fglob)) {
        /* error */
        return 1;
    }

    /* set up the times */
    if (fglobInitTimes(fglob)) {
        /* error */
        return 1;
    }

    /* parse the classes and types */
    if (fglobParseClassAndType(fglob)) {
        /* error */
        return 1;
    }

    /* now parse the sensor list and check w.r.t. the class/type. */
    if (fglobParseSensors(fglob, NULL)) {
        /* error */
        return 1;
    }
...
fglobHandler(
    ...
    char               *opt_arg)
{
    sk_fglob_t *fglob = (sk_fglob_t *)cData;

    assert(fglob);

    ++fglob->fg_user_option_count;
    ...
    switch (opt_index) {
      ...
      case FGLOB_OPT_END_DATE:
        if (fglob->fg_option[opt_index] != NULL) {
            skAppPrintErr("Invalid %s: Switch used multiple times",
                          fglobOptions[opt_index].name);
            return 1;
        }
        fglob->fg_option[opt_index] = opt_arg;
        break;
    ...
    sk_fglob_t         *fglob)
{
    ...
    fglob->fg_flowtype_list = sk_alloc_array(sk_flowtype_id_t, flowtype_count);
    fglob->fg_sensor_list = sk_alloc_array(int *, flowtype_count);
    for (i = 0; i < flowtype_count; ++i) {
        fglob->fg_sensor_list[i] = sk_alloc_array(int, sensor_count + 1);
    }
    fglob->fg_sensor_count = sk_alloc_array(int, flowtype_count);
    ...
    sk_fglob_t         *fglob)
{
    ...
    rv = sksiteRepoIteratorParseTimes(&start_val, &end_val,
                                      fglob->fg_option[FGLOB_OPT_START_DATE],
                                      fglob->fg_option[FGLOB_OPT_END_DATE],
                                      &error_code);
    switch (rv) {
      case 0:
        /* parse was successful */
        fglob->fg_time_start = start_val;
        fglob->fg_time_end = end_val;
        /* Set the current time to the starting value. */
        fglob->fg_time_idx = fglob->fg_time_start;
        break;
      case 1:
    ...
    sk_fglob_t         *fglob)
{
    ...
    while (sksiteSensorIteratorNext(&si, &sensor_id)) {
        ...
        while (sksiteClassIteratorNext(&ci, &class_of_sensor)) {
            ...
            for (k = 0; k < fglob->fg_flowtype_count; ++k) {

                /* if the class of the flowtype equals
                 * 'class_of_sensor', we will search over that
                 * sensor */
                if (sksiteFlowtypeGetClassID(fglob->fg_flowtype_list[k])
                    == class_of_sensor)
                {
                    /* add this sensor to this flowtype */
                    fglob->fg_sensor_list[k][fglob->fg_sensor_count[k]++] =
                        sensor_id;
                }
            }
        }
 ...
 *    input that is specified in the fglob global variable.  If the
    ...
    sk_bitmap_t       **sensor_bits_ptr)
{
    ...
    if (NULL == fglob->fg_option[FGLOB_OPT_SENSORS]) {
        assert(NULL == sensor_bits_ptr);
        fglobEnableAllSensors(fglob);
        return 0;
    }
    ...
    rv = sksiteParseSensorList(sensors_vec,fglob->fg_option[FGLOB_OPT_SENSORS],
    ...
    if (rv) {
        if (rv < 0) {
            ...
            skAppPrintErr("Invalid %s '%s': %s",
                          fglobOptions[FGLOB_OPT_SENSORS].name,
                          fglob->fg_option[FGLOB_OPT_SENSORS],
    ...
    for (i = 0; 0 == skVectorGetValue(&id, sensors_vec, i); ++i) {
        fglobAddSensor(fglob, sensor_bits, id, (sensor_bits_ptr == NULL));
    }
        ...
        && (sensor_bits_ptr == NULL))
    {
        ...
        for (i = 0; i < (size_t)fglob->fg_flowtype_count; ++i) {
            if (0 == fglob->fg_sensor_count[i]) {
                sksiteFlowtypeGetClass(class_name, sizeof(class_name),
                                       fglob->fg_flowtype_list[i]);
    ...
    sk_flowtype_id_t    flowtype_id)
{
    ...
    for (j = 0; j < fglob->fg_flowtype_count; ++j) {
        if (flowtype_id == fglob->fg_flowtype_list[j]) {
            return;
        }
    }

    fglob->fg_flowtype_list[fglob->fg_flowtype_count] = flowtype_id;
    ++fglob->fg_flowtype_count;
}
    ...
    sk_fglob_t         *fglob)
{
    ...
    rv = sksiteParseFlowtypeList(flowtypes_vec,
                                 fglob->fg_option[FGLOB_OPT_FLOWTYPES],
    ...
    sk_fglob_t         *fglob)
{
    ...
    if (fglob->fg_flowtype_count > 0) {
        return 0;
    }

    /* handle case when --flowtypes is given */
    if (fglob->fg_option[FGLOB_OPT_FLOWTYPES]) {
        if (fglob->fg_option[FGLOB_OPT_CLASS]
            || fglob->fg_option[FGLOB_OPT_TYPE])
        {
        ...
        return fglobParseFlowtypes(fglob);
    }

    /* parse --class or use default */
    class_name = fglob->fg_option[FGLOB_OPT_CLASS];
    if (NULL == class_name) {
    ...
    if (NULL == fglob->fg_option[FGLOB_OPT_TYPE]) {
    ...
    sk_fglob_t         *fglob)
{
    ...
    if (fglob->fg_user_option_count == 0) {
    ...
    sk_bitmap_t       **flowtype_bitmap)
{
    ...
    if (fglob->fg_option[FGLOB_OPT_SENSORS]) {
        /* ensure site configuration file */
        if (fglobLoadSiteConfig(fglob)) {
            return -1;
        }

        if (fglobParseSensors(fglob, sensor_bitmap)) {
            return -1;
        }
        ...
        || fglob->fg_option[FGLOB_OPT_FLOWTYPES])
    {
        ...
        for (i = 0; i < fglob->fg_flowtype_count; ++i) {
            skBitmapSetBit(*flowtype_bitmap, fglob->fg_flowtype_list[i]);
        }
    ...
    if (fglob->fg_option[FGLOB_OPT_TYPE]) {
        --fglob->fg_user_option_count;
    }
    if (fglob->fg_option[FGLOB_OPT_FLOWTYPES]) {
        --fglob->fg_user_option_count;
