...
These plug-ins must be explicitly loaded into an application using the
application's B<--plugin> switch.

B<flowkey(3)>. The flowkey plug-in, which must be loaded explicitly,
adds a switch and a field that computes a 32-bit hash for a flow
record using the same algorithm as YAF uses for its flow key utility
B<getFlowKeyHash(1)>.  I<Since SiLK 3.15.0.>
...
suite.  The variables are listed alphabetically.
...
is not provided on the command line.  I<Since SiLK 3.7.0.>
...
$SiLK: silk.pod 7f87df5c3e66 2017-03-24 21:16:58Z mthomas $
