...
Note that the SiLK flow records represent the sensor and flowtype by
their numeric IDs, not by their names.
...
associated with a sensor name as determined by its configuration
...
creates.  Changes to the C<silk.conf> and B<rwflowpack> configuration files may
...
tools and the packing tool B<rwflowpack(8)>.
...
used by B<rwflowpack> when writing data to the
...
short, to declare that these sensors have data for this class.
Sensors may be listed by name or numeric ID, and sensor groups may
also be added by prefixing their names with an at (C<@>) character.
When you add a sensor group, it is the same as individually adding
each sensor in that group to the class.
...
command within a group block.  Sensors may be listed by name or
numeric ID, and sensor groups may also be added by prefixing their
names with an at (C<@>) character.  When you add a sensor group, it is
the same as individually adding each sensor to the group being
defined.
...
B<rwfilter(1)>, B<rwfglob(1)>, B<rwsiteinfo(1)>,
B<rwflowpack(8)>, B<silk(7)>,
I<SiLK Installation Handbook>

=cut

$SiLK: silk.conf.pod 2388e0826cdc 2016-07-20 16:31:42Z mthomas $
