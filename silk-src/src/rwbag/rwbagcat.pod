...
B<--network-structure> or B<--sort-counters> switches.
...
value.  To sorted based on the counter value, specify the
B<--sort-counter> switch.  The default sort order is from maximum
counter to minimum counter.
...
The order of the keys is consistent for keys have the same counter
value.  The following output is limited to those keys whose value is
...
$SiLK: rwbagcat.pod 66e65613d494 2017-03-21 19:29:56Z mthomas $
