...
when the B<--coverset> switch is used.  Valid values are 0, 2, 3, and
4.  When the switch is not provided, the SILK_IPSET_RECORD_VERSION
environment variable is checked for a version.  A I<VERSION> of 2
creates a file compatible with S<SiLK 2.x>, and it can only be used
for IPsets containing IPv4 addresses.  A I<VERSION> of 3 creates a
file that can only be read by S<SiLK 3.0> or later.  A I<VERSION> of 4
creates a file that can only be read by S<SiLK 3.7> or later.  Version
4 files are smaller than version 3 files.  The default I<VERSION> is
0, which uses version 2 for IPv4 IPsets and version 3 for IPv6
IPsets. I<Since SiLK 3.11.0>.
...
$SiLK: rwbagtool.pod db70b3edb445 2015-09-08 22:05:17Z mthomas $
