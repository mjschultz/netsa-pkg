...
RCSIDENTVAR(rcsID_RWASCII_H, "$SiLK: rwascii.h efd886457770 2017-06-21 18:43:23Z mthomas $");
...
/**
 *    Name of the enum holding the RWREC_FIELD_XXX identifiers before
 *    the enum was moved to rwrec.h
 */
#define rwrec_printable_fields_t  rwrec_field_id_t

/**
 *    Number of fields we can print; should be one more than the last
 *    ID in rwrec_printable_fields_t
 */
#define  RWREC_PRINTABLE_FIELD_COUNT  RWREC_FIELD_ID_COUNT
 ...
 *
 *    When rwAsciiPrintRecExtra(astream, rec, extra) is called, the
 *    AsciiStream will invoke get_value_extra_fn(rec, buf, bufsize,
 *    callback_data, extra) to generate the value for the field.
 */
int
