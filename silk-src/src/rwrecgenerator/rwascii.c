...
RCSIDENT("$SiLK: rwascii.c efd886457770 2017-06-21 18:43:23Z mthomas $");
...
/* default capacity of the array of fields */
#define RWASCII_DEFAULT_FIELD_CAPACITY   32
    ...
    uint32_t            capacity)
{
    ...
    if (capacity > 0) {
        ...
        capacity = RWASCII_DEFAULT_FIELD_CAPACITY;
    } else {
 ...
 *    caller did not choose specific columns, and set the width of the
 *    columns.
 */
static void
rwAsciiPreparePrint(
    ...
    void               *extra)
{
         ...
         ++i, ++field)
    {
        ...
        switch (field->af_field_id) {
          ...
          case RWREC_FIELD_PKTS:
            sprintf(buffer, ("%" PRIu64), rwRecGetPkts(rwrec));
            break;

          case RWREC_FIELD_BYTES:
            sprintf(buffer, ("%" PRIu64), rwRecGetBytes(rwrec));
            break;
    ...
    sk_stringmap_t    **field_map)
{
    ...
    /* add entries */
    if (skRwrecAppendFieldsToStringMap(*field_map)) {
