...
One or more partitioning switches are required:
  ...
  rwfilter ...
        ...
        [--lua-expression=LUA_EXPR [--lua-expression=LUA_EXPR ...]]
        [--lua-file=FILENAME [--lua-file=FILENAME ...]]
...
files listed on the command line or from the B<--xargs>
switch, the first four switches (B<--class>,
...
input, specify C<stdin> or C<-> as the name of an input file.
...
the packing tools were configured to capture it.
...
data repository (that is, from files listed on the command line or from
files specified by B<--xargs>), these
...
B<Partitioning Switches that use Lua>

=over 4

=item B<--lua-file>=I<FILENAME>

Load and run the Lua code in the file named I<FILENAME> during set-up.
The switch may be repeated to load multiple files.  The user's Lua
code may call the B<register_filter()> function whose first argument
is a filtering function.  The filtering function should accept a
single argument that is a Lua B<rwrec> object (see B<silklua(1)>).
The filtering function is called for every record unless a previous
partitioning rule determines that the record has failed.  The record
is still considered a pass candidate unless the filtering function
returns a value of B<false> or B<nil>.

The B<register_filter()> function accepts a second optional argument
that is a table.  The table may have a C<initialize> key and function
value; this function is called after argument processing and before
processing records.  The table may also have a C<finalize> key and a
function value; this function is called during shutdown.

=item B<--lua-expression>=I<LUA_EXPR>

Use the return value of I<LUA_EXPR> as a potential pass/fail
determiner.  The current flow record is bound to the variable C<rec>
prior to invoking the expression.  The record is still considered a
pass candidate unless I<LUA_EXPR> evaluates to B<false> or B<nil>.
...
A Lua expression may be used to partition records.  The following
example reads the file F<data.rw> and reports the number of TCP
records whose source port is less than 1024.

 $ rwfilter --lua-expression='rec.protocol==6 and rec.sport<1024' \
         --print-statistics data.rw
 Files     1.  Read     501876.  Pass     157444. Fail      344432.

The contents of the F<filter.lua> file does the same thing:
 ...
 function report_pass ()
   print(string.format("Read %10d. Pass %10d. Fail %10d.",
                       pass + fail, pass, fail))
 end
 function check_port (r)
   if r.protocol == 6 and r.sport < 1024 then
     pass = pass + 1
   else
     fail = fail + 1
   end
 ...
 register_filter(check_port, {finalize = report_pass})

The B<register_filter()> function is used to register a function that
gets called for each record, and a finalize function is used to report
the results.  B<rwfilter> requires that an output switch be specified,
and the B<--print-statistics> is used, but since the Lua function does
not return a value, B<--print-statistics> reports that no records
pass.

 $ rwfilter --lua-file=filter.lua --print-stat tests/data.rwf
 Files     1.  Read     501876.  Pass          0. Fail      501876.
 Read     501876. Pass     157444. Fail     344432.

The following Lua file registers a function that passes records where
the record has sidecar data that includes a C<dnsQName> field whose
value includes the string C<google>.  Since not every record has
sidecar data, the function must check whether the record has a
sidecar, since otherwise Lua complains about an attempt to index a nil
value.  Another check is made to ensure the sidecar data includes the
C<dnsQName> field.

 register_filter(function (r)
   if r.sidecar and r.sidecar.dnsQName then
     return string.find(r.sidecar.dnsQName, "google")
   end
 end)
...
Prior to SiLK 4.0, B<rwfilter> supported an B<--input-pipe> switch whose
argument was either the path to a named pipe from which B<rwfilter> read
records or the strings C<stdin> or C<-> to tell B<rwfilter> to read from
the standard input.  The switch was removed since it was unnecessary.
Instead simply specify the path to the named pipe or the string
C<stdin> or C<-> as a command-line argument.
...
B<rwtuc(1)>, B<rwsetbuild(1)>, B<rwsiteinfo(1)>, B<silklua(1)>,
B<rwpmapbuild(1)>, B<addrtype(3)>, B<ccfilter(3)>, B<flowrate(3)>,
B<ipafilter(3)>, B<pmapfilter(3)>, B<pysilk(3)>, B<silkpython(3)>,
B<silk-plugin(3)>, B<silk.conf(5)>, B<silk(7)>, B<rwflowpack(8)>,
...
$SiLK: rwfilter.pod 6ced2b9cf93b 2017-06-26 21:15:05Z mthomas $
