...
=head1 SYNOPSIS
  ...
  rwstats [--pmap-file=MAPNAME:PATH [--pmap-file=MAPNAME:PATH ...]]
        [--plugin=PLUGIN ...] [--lua-file=PATH ...]
        [--python-file=PATH ...] --help

  rwstats [--pmap-file=MAPNAME:PATH [--pmap-file=MAPNAME:PATH ...]]
        [--plugin=PLUGIN ...] [--lua-file=PATH ...]
        [--python-file=PATH ...] --help-fields
...
column is printed.  The percentage columns contain a question mark
when the primary aggregate value is either a distinct count or comes
from a plug-in since B<rwstats> does not know whether summing the
aggregate values is reasonable.  The display of the percentage columns
may be suppressed by specifying B<--no-percents>.
...
specified using the the B<--fields> switch.
...
be certain to include the C<protocol> field as part of your key.
...
or Bottom-N.  If the B<--values> switch is not specified, B<rwstats> counts the
...
B<--fields>.  For example,
C<Distinct:sPort> counts the number of distinct source ports for each
bin.  When this aggregate value field is used, the specified
I<KEY_FIELD> may not be present in the argument to B<--fields> (since
the distinct count ni that case is one).
...
=item B<--lua-file>=I<PATH>

Load the Lua code from the file I<PATH>.  This Lua code may define
additional sidecar fields on each record that may be used as part of
the key in B<--fields> or as a distinct value in B<--values>.  The
switch may be repeated to load Lua code from multiple files.
...
not be present and the primary value must be C<Bytes>, C<Packets>, or
C<Records>.  If B<rwstats> runs out of memory
...
#### This example requires code that has not yet been copied from the
#### default branch.
...
B<pysilk(3)>, B<silkpython(3)>, B<silk-plugin(3)>, B<rwflowpack(8)>,
B<silk(7)>, B<yaf(1)>, B<dlopen(3)>, B<tzset(3)>, B<environ(7)>

=cut

$SiLK: rwstats.pod 12885affd439 2017-06-26 22:07:47Z mthomas $
