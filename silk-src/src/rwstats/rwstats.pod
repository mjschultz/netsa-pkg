...
inclusive.  To use this switch, the primary value must be C<Bytes>,
C<Packets>, or C<Records>, and the B<--presorted-input> switch must
not be present.  If B<rwstats> runs out of memory while locating all
bins that meet the percentage, B<rwstats> prints the Top-N or Bottom-N
using the amount of memory it was able to allocate.  The value of
I<PERCENT> was required to be a positive integer prior to SiLK 3.12.0,
...
specified, B<rwstats> will merge-sort the flow records from the input
...
$SiLK: rwstats.pod 57cd46fed37f 2017-03-13 21:54:02Z mthomas $
