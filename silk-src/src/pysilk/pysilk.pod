...
As of SiLK 4.0.0, the B<IPAddr> constructor does not accept an integer
I<address>.  Use the B<IPv4Addr> and B<IPv6Addr> constructors instead.
...
vlanId if the packing tools are configured to capture it, a 16-bit
integer.  The default input value is 0.
...
postVlanId if the packing tools are configured to capture it, a 16-bit
integer.  The default output value is 0.
...
C<E>, and C<C>---upper or lower-case---representing the TCP_FIN,
TCP_SYN, TCP_RST, TCP_PSH, TCP_ACK, TCP_URG, TCP_ECE, and TCP_CWR
...
B<rwgeoip2ccmap(1)>, B<silk.conf(5)>, B<silk(7)>,
B<python(1)>, B<gzip(1)>, B<yaf(1)>, B<tzset(3)>,
L<http://docs.python.org/>

=cut

$SiLK: pysilk.pod 5036d9fcc2e7 2017-06-26 20:31:01Z mthomas $
