...
L<http://www.iana.org/cctld/cctld-whois.htm>) or one of the following
special codes:
...
MaxMind, available from L<http://www.maxmind.com>, as described in the
L</MAPPING FILE> section below.
...
L<http://www.maxmind.com/>.  We do not distribute the database nor the
data file, but we provide Perl scripts that will convert the GeoIP
database to the format that F<ccfilter.so> expects.

MaxMind distributes multiple versions of their GeoIP Country database;
one is a free evaluation copy that is C<97% accurate>.  In addition,
they sell versions with higher accuracy, and they offer various
subscription services.

The B<rwgeoip2ccmap(1)> program converts the MaxMind GeoIP file to the
...
$SiLK: ccfilter.pod 9e71f5672f24 2013-03-05 23:37:50Z mthomas $
