...
B<rwgeoip2ccmap> - Create a country code prefix map from a GeoIP CSV file
...
I<legacy> comma separated value (CSV) database format.
B<rwgeoip2ccmap> does not support the GeoIP2 and GeoLite2 databases,
and the binary forms of the database are no longer supported as of
SiLK 3.12.0.)

The database is available in two supported formats:
...
=item B<--mode=>{B<auto>|B<ipv4>|B<ipv6>}

Specify the type of prefix map file to create.  When not specified,
B<rwgeoip2ccmap> determines the type of prefix map to create based on
the first line of input.  The modes are:
...
Create an IPv4 prefix map.  Any IPv6 addresses in the ::ffff:0:0/96
netblock are mapped to an IPv4 address, and all other IPv6 addresses
are ignored.

=item B<ipv6>

Create an IPv6 prefix map.  Any IPv4 addresses are mapped into the
...
Read the CSV form of the GeoIP Legacy country code database from
...
Print an error message and exit.  The binary form of the database is
no longer supported as of SiLK 3.12.0, and the CSV form of the GeoIP
Legacy database must be used instead.
...
Download CSV version of the MaxMind GeoIP I<Legacy> Country database
...
$SiLK: rwgeoip2ccmap.pod c24c943e8d9f 2016-04-01 20:55:25Z mthomas $
